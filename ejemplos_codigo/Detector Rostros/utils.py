...
def detections_by_scale(
    test_image, test_scales, step, clf, scaler, pca, size=(64, 64), thresholds=[0.5]
):
    raw_detections = []
    detections = []
...
def evaluate_detections_by_scale_2(
    image,
    test_scales,
    clf,
    # scaler,
...
def global_multiscale_detection(
    image,
    clf,
    scaler,
    pca,
    ...
    for scale in tqdm(test_scales, desc="Escalas"):
        ...
        indices, patches = zip(
            *sliding_window(image, scale=scale, istep=step, jstep=step)
        )
        patches_hog = np.array([feature.hog(patch) for patch in patches])
        patches_hog = scaler.transform(patches_hog)
        patches_hog = pca.transform(patches_hog)
    ...
    if plot:
        ...
        for (i, j), (Ni, Nj) in zip(filtered_indices, filtered_sizes):
            ax.add_patch(
                plt.Rectangle(
                    (j, i), Nj, Ni, edgecolor="blue", alpha=0.6, lw=1, facecolor="none"
                )
            )
...
def global_multiscale_detection_2(
    image,
    clf,
    # scaler,
    pca,
...
def evaluate_detections_at_scale(
    image,
    clf,
    scaler,
    pca,
    ...
    if plot:
        ...
        for i, thr in enumerate(thresholds):
            ...
            for y, x in detecciones_dict[thr]:
                ax[i].add_patch(
                    plt.Rectangle(
                        (x, y), Nj, Ni, edgecolor="red", alpha=1, lw=1, facecolor="none"
                    )
                )
...
def evaluate_detections_at_scale_2(
    image,
    clf,
    # scaler,
    pca,
